        ...
        if (Platform.OS === 'android') {
            try {
                ...
                if (
                    ...
                    Alert.alert('Permissions Denied', 'Please allow camera and storage permissions to use this feature.');
                }
            } catch (err) {
                console.warn(err);
            }
    ...
    const openImagePicker = () => {
        ...
        launchImageLibrary(options, (response) => {
            if (response.didCancel) {
                ...
                Alert.alert('Image Picker Error', 'There was an error while picking an image. Please try again.');
            } else {
    ...
    const handleCameraLaunch = () => {
        ...
        launchCamera(options, (response) => {
            if (response.didCancel) {
                ...
                // Display an error message to the user
                Alert.alert('Camera Error', 'There was an error while opening the camera. Please try again.');
            } else {
                let imageUri = response.uri || response.assets?.[0]?.uri;
                setSelectedImage(imageUri);
            }
